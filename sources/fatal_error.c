...
int8* getFatalErrorDescription(enum FatalErrorsType errorType) {
	int8* errorMessages[] = {
		...
		[BadTSError] = "Your program is extremely nervous, please calm down your brainchild.\n\n",
		[SegmentTSNotFoundError] = "Nerves are not a gift, but they are needed more than ever for a person (and preferably for programs).\n\n",
	...
	if (errorType <= sizeof(errorMessages) / sizeof(int8*)) return errorMessages[errorType];

	else return "There are no words.\n\n";
}
