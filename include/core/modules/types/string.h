...
static inline void substr(String* _result, String* x, uint32 start, uint32 end) {
	assert(!isValidMemoryRegion(x->region), Exception_fromError(INVALID_ARGUMENT_ERROR, Exception_messageFromString("Cannot cut a substring from a incorrect string.")));
	
	assert(end > x->length, Exception_fromError(INVALID_ARGUMENT_ERROR, Exception_messageFromString("The specified size is greater than the string size=%i to the end=%i, the code execution will continue, but it is necessary to check the correctness of the arguments.", x->length, end)));
