...
static inline double mod(double a, double b) {
	...
	assert(b == 0.0, Exception_fromError(ERROR_ZERO_DIVISION, Exception_messageFromString("Cannot mod [dbl] with 0.", a)));
...
static inline uint32 ifloorU32(uint32 x, uint32 align) {
	assert(align == 0, Exception_fromError(DIVISION_BY_ZERO_ERROR, Exception_newMessage(__FILE__, __LINE__, "Cannot floor align x with 0.")));
...
static inline uint32 iceilU32(uint32 x, uint32 align) {
	assert(align == 0, Exception_fromError(DIVISION_BY_ZERO_ERROR, Exception_messageFromString("Cannot ceil align x with 0.")));
